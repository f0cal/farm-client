...
def resolve_remote_url(remote_name):
    remotes_file = JsonFileParser(core.CORE.config['api']['remotes_file'])
...
def create_class(class_name, noun, remote=False):
    ...
    if remote:
        ...
        api_url = core.CORE.config["api"]["api_url"]
...
def query(class_name, noun, ref, remote=None):
    ...
    try:
        if ref_type == 'name':
            ...
            if noun == 'instance':
                device_config = JsonFileParser(core.CORE.config['api']['device_file'])
